...
class PeerPanel extends ConsumerWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                peersAsync.when(
                  data: (_) => Icon(Icons.wifi, size: 16, color: Colors.green.shade600),
                  loading: () => SizedBox(
                    width: 16, height: 16,
                    child: CircularProgressIndicator(strokeWidth: 2, color: colorScheme.primary),
                  ),
                  error: (_, _) => Icon(Icons.wifi_off, size: 16, color: colorScheme.error),
                ),
              ],
            ),
          ),
          ...
          Expanded(
            child: _buildPeerList(context, ref, peersAsync, dbPeersAsync, selectedPeerId, colorScheme, theme),
          ),
        ],
      ),
    );
  ...
  Widget _buildPeerList(
    ...
    String? selectedPeerId, ColorScheme colorScheme, ThemeData theme,
  ) {
    ...
    for (final entry in discoveredPeers.entries) {
      ...
      allPeers[entry.key] = _PeerInfo(
        id: peer.deviceId, name: peer.deviceName, os: peer.os,
        isOnline: true, ip: peer.ipAddress, port: peer.port,
      );
    }
    for (final peer in dbPeers) {
      if (!allPeers.containsKey(peer.id)) {
        allPeers[peer.id] = _PeerInfo(
          id: peer.id, name: peer.deviceName, os: peer.os,
          isOnline: false, ip: peer.ipAddress, port: peer.port,
        );
      }
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _PeerTile(
          peer: peer, isSelected: peer.id == selectedPeerId,
...
class _PeerInfo {
  ...
  final String? ip;
  final int? port;
  _PeerInfo({required this.id, required this.name, required this.os, required this.isOnline, this.ip, this.port});
}
...
class _PeerTile extends StatelessWidget {
  ...
  const _PeerTile({required this.peer, required this.isSelected, required this.onTap, required this.colorScheme, required this.theme});
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: peer.isOnline ? Colors.green.shade600 : Colors.grey.shade400,
                ),
              ),
              const SizedBox(width: 10),
              Icon(_osIcon(peer.os), size: 20,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      peer.isOnline ? '${peer.os} - ${peer.ip}' : '${peer.os} (offline)',
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: colorScheme.onSurfaceVariant.withValues(alpha: 0.7), fontSize: 11),
                      overflow: TextOverflow.ellipsis),
                  ],
                ),
              ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Scanning LAN...', style: theme.textTheme.bodyMedium?.copyWith(
              color: colorScheme.onSurfaceVariant), textAlign: TextAlign.center),
            const SizedBox(height: 4),
            Text('Devices running LANShare\nwill appear here',
              style: theme.textTheme.bodySmall?.copyWith(
